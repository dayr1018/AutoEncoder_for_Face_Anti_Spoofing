...
def plot_real_fake_data(path, filename, epoch, y1, y2):
    ...
    fig = plt.figure()
    plt.title(f"Data Distrbution (Normal, Abnomal) - Epoch {epoch}")
    plt.xlabel("Data Number")
    plt.ylabel("Reconstruction Error")
    plt.plot(x1, y1, 'bo', label=f"Normal Data({len(y1)})")
    plt.plot(x2, y2, 'ro', label=f"Abnomal Data({len(y2)})")
    plt.legend(loc='upper right', fontsize='x-small')
    plt.savefig(path+f'/{filename}.png')
    plt.close(fig)
...
def plot_histogram(path, filename, epoch, real, fake):
    ...
    plt.ylabel("Number")
    plt.title(f"Histogram (Normal, Abnomal) - Epoch {epoch}")
    plt.hist((real, fake), label=('Normal', 'Abnomal'))    
    plt.legend(loc='upper right', fontsize='x-small')
    plt.savefig(path+f'/{filename}.png')
    plt.close(fig)
